...
Figures are generated from `02_Data/portfolio_ledger.json` and marked as of 2026-09-12. Invested capital is 9,229.92 USD, marked at 9,032.98 USD, unrealized -2.13%, across 44 positions and 80 sealed units.

**Marking policy.** A position is marked only from an observed marketplace sale price (TCGplayer market, latest observation in the daily `price_history.parquet` lake or the weekly `tcgcsv_prices/` archive). Dealer asks and dealer bids are not substituted for a market price. A position the feed does not cover is carried at cost and flagged `priced: false`, shown as "at cost" and asterisked in Table 2. Currently 36 of 44 positions, 94.2% of cost basis, are marked to an observed price; the remainder sit at cost. Holdings sum to the bucket totals and to the summary line, and both tables render an explicit total row computed from the holdings. That identity is the first thing to re-check after any edit.
...
The monthly path is rebuilt from the lake rather than interpolated. Each month end values a position held on that date at the latest observed tcgplayer market price on or before it, and at cost where no observation existed yet. The weekly archive starts on 2024-02-08 and the daily snapshots on 2026-08-28; before a product's first observation it sits at cost, so stretches of the early path are flat by construction, not by luck. There are no intramonth marks. The first acquisition is 2024-06-06, so the pre-November-2024 section of Exhibit 2 sits on a single 51.30 USD position and the percentages there are not comparable with later periods.
